...
void termFunc(int termNum)
{
    ...
    if (termNum == Term1) {
        testReceiverY(daSktPrT1M[Term1]);        // file does not exist
       // testReceiverY(daSktPrT1M[Term1]);        // empty file and normal file.
        pthreadSupport::setSchedPrio(20);
        PE(myClose(daSktPrT1M[Term1]));
        ...
        //testSenderY(iFileNamesA, daSktPrMT2[Term1]);   // file does not exist
        testSenderY(iFileNamesB, daSktPrMT2[Term1]);   // empty file and normal file.
        pthreadSupport::setSchedPrio(20); // drop priority down somewhat.
        PE(myClose(daSktPrMT2[Term1]));
    }
}
